...
def mh_to_png(filename, path):
    mha = sitk.ReadImage(filename+".mha")
...
def mhd_to_mha(image_path, filename_output_mha, filename_output_label_split, filename):
    image_arr, spacing, origin  = load_itk(image_path)
    #image_arr = np.transpose(image_arr, (1, 2, 0))
    #image_arr_added = add_laser_layer_bin(image_arr, filename_output_label_split, filename)
...
def load_ALL_label(filepath_input, filename_output_mha, filename_output_mhd, type_mod, filename_output_label_split, filename_output_mha_nonbin, filename_output_mha_bin, filename_output_mha_self):
    ...
    for filename in filepath:
        if filename.endswith("oCams.mhd"):
            ...
            image = mhd_to_mha(image_path, filename_output_mha_nonbin, filename_output_label_split, filename)
        if filename.endswith("binarized-10.mhd"):
            image_path = os.path.join(filepath_input, filename)
            image = mhd_to_mha(image_path, filename_output_mha_bin, filename_output_label_split, filename)
            sitk.WriteImage(image,filename_output_mha)
            sitk.WriteImage(image,filename_output_mhd)
        if filename.startswith(type_mod):
            if filename.endswith(".mhd"): 
                image_path = os.path.join(filepath_input, filename)
                image_arr, spacing, origin  = load_itk(image_path)
                array_laser = add_laser_layer(image_arr, filename_output_label_split, filename)
                arrays.append(array_laser)
    arrays_combined = np.array(arrays)
    image_combined = sitk.GetImageFromArray(arrays_combined)
    sitk.WriteImage(image_combined,filename_output_mha_self)
...
def for_loop(path_saving, name1, dest_dir_train, dest_dir_train_label, dest_dir_test, dest_dir_test_label):
    type_mod_data = "FMI-cfLights-surfEm-oCam-"
    type_mod_label = "BLI-oCam-c"
    i = 0
...
directory_home = '/home/svea/lipros/lipros_training/training_set_05/'
#directory_home = '/home/svea/lipros/lipros_training/training_set_02/output_simulation/'
source_directory_simulation = directory_home+'2024-03-08-lipros-30-sphere-for-svea'
source_directory_label = source_directory_simulation+'/neu-2024-03-08-15-31-02-lipros-30-sphere-BLI'
source_directory_data = source_directory_simulation+'/neu-2024-03-08-16-22-30-lipros-30-sphere-FMI'
destination_directory_train = directory_home+'train'
destination_directory_train_label = directory_home+'train_label'
destination_directory_test = directory_home+'testing'
destination_directory_test_label = directory_home+'testing_label'

#split_data_in_directory(source_directory_simulation, source_directory_data, source_directory_label)
copy_every_third_directory(source_directory_data, source_directory_label, destination_directory_train, destination_directory_train_label, destination_directory_test, destination_directory_test_label)
 
#________prepare data as input for nnUNet2________
path_saving = '/home/svea/Documents/nnUNetv2/nnUNet_raw/Dataset026_CTpositionRadon'
name_saving = "Dataset026"
